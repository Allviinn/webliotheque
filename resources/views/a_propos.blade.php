<!DOCTYPE html>
<html lang="{{ config('app.locale') }}">
    <head>
        <meta charset="utf-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <title>À propos</title>
        <link rel="stylesheet" type="text/css" href="css/styleJulie/style_a_propos.css">
    </head>

    <body>
        <h2>L'équipe du projet</h2>
        <div id="fleche"><a href="webliotheque/public/"><img src="css/styleJulie/fleche.png"></a></div>
        <p id='description'>Nous sommes une petite équipe de 4 personnes SURMOTIVÉES !</p><br><br><br>

    <div>
        <div class="description">
            <img src="css/styleJulie/alvin.png">
            <h4>Alvin</h4>
            <p>Sa passion, manger bien-sûr,
            alors faites attention à votre repas ! <br>
            Ou sinon prenez lui une crêpe au Nutella&nbsp;&nbsp;<img src="css/styleJulie/eyes.png"></p>
        </div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;

        <div class="description">
            <img src="css/styleJulie/florian.png">
            <h4>Florian</h4>
            <p> Sa&nbsp;&nbsp;<img src="css/styleJulie/gamepad.png"> c'est toute sa vie !<br>À lui seul on dénombre pas moins de 10 000 000 monstres tués !</p>
        </div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;

        <div class="description">
            <img src="css/styleJulie/raphael.png">
            <h4>Raphaël</h4>
            <p>Il est esclave et ami des&nbsp;&nbsp;<img src="css/styleJulie/cat.png">&nbsp;&nbsp;<br>lui aussi est amateur de bons petits plats !</p>
        </div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;

        <div class="description">
            <img src="css/styleJulie/julie.png">
            <h4>Julie</h4>
            <p>Elle est extrêmement chiante .<br>Attend encore sa lettre pour Poudlard !&nbsp;&nbsp;&nbsp;<img src="css/styleJulie/love.png"></p>
        </div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    </div>
    </body>
    <footer>
        <p>Fait dans la joie et la bonne humeur avec beaucoup de <img src="css/styleJulie/coffee.png"> et pas mal de <img src="css/styleJulie/pancake.png"> !
    </footer>
</html>
